...
OrestesInserter.prototype.validate_and_handle_metadatum = function(pt, bucket) {
    bubo.add(this.space + '@' + bucket, pt, buboResult);
    ...
    if (!buboResult.found) {
        try {
            ...
            bubo.delete(this.space + '@' + bucket, pt);
            throw err;
        }
    } else {
        validateValue(pt);
...
OrestesInserter.prototype.remove_metadata_from_bubo = function() {
    for (var k = 0; k < this.metadata.length; k += 1) {
        ...
        bubo.delete(this.space + '@' + bucket, metadatum);
    }
};
...
OrestesInserter.prototype.insert_metadata = function() {
    ...
    .spread(function(res, body) {
        JSON.parse(body).items.forEach(function(item, i) {
            if (item_is_failure(item)) {
                var failed_metadatum = JSON.parse(self.metadata[i]);
                bubo.delete(self.space + '@' + self.buckets[i], failed_metadatum);
                self.errors.push({pt: failed_metadatum, error: item.create ? item.create.error : 'unknown'});
            }
        });
    })
    .catch(function(err) {
